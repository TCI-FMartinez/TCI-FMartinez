...
def load_and_preprocess_data(file_path):
    ...
    df = pd.read_csv(file_path, delimiter=";", header=None, names=ns_list,
                 encoding="utf-8", engine="python", on_bad_lines='skip')
    
    # Mapeo de columnas según estructura conocida
    column_mapping = {
        'material': 2,
        'thickness': 4,       # N004
        'laser_power': 5,     # N005
        'feedrate_e01': 11,   # N086
        "Focal calidad1": 23,
        'feedrate_e02': 17,   # N098
        "Focal calidad2": 25,
        'gas_pressure': 15,   # N090
        'nozzle_diameter': 9  # N012
    }

    # Para la normalización de nombres de materiales
    material_map = {
    ...
    def normalize_material(name):
        if pd.isna(name):  # Si es NaN, retornar sin cambios
            return name
        name = str(name).strip().lower()  # Convertir a minúsculas y quitar espacios
        name = unidecode.unidecode(name)  # Remover acentos
        return material_map.get(name, name)  # Sustituir si está en el diccionario

    # Aplicar la normalización a la columna de materiales
    #df['material'] = df['material'].apply(normalize_material)

    # Extraer y limpiar columnas relevantes
    data = []
    for _, row in df.iterrows():
        try:
            entry = {
                'material': material_map.get(str(row[column_mapping['material']]).strip().lower(), "desconocido"),
                'thickness': float(row[column_mapping['thickness']]),
                'laser_power': int(row[column_mapping['laser_power']]),
                'feedrate_e01': int(row[column_mapping['feedrate_e01']]),
                "focal_calidad1": float(row[column_mapping['Focal calidad1']]),
                'feedrate_e02': int(row[column_mapping['feedrate_e02']]),
                "focal_calidad2": float(row[column_mapping['Focal calidad2']]),
                'gas_pressure': float(row[column_mapping['gas_pressure']]),
                'nozzle_diameter': float(row[column_mapping['nozzle_diameter']])
            }
            data.append(entry)
...
def analyze_materials(df):
    """
    Genera estadísticas básicas agrupadas por material y espesor.
    """
    if df.empty:
...
def main():
    ...
    if not desconocidos.empty:
        ...
        print("\nEjemplos de registros desconocidos:")
        print(desconocidos.sample(min(5, len(desconocidos))).to_string(index=False))
    exit(0)
