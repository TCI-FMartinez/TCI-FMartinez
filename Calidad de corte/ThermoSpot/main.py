...
def demanda_termica_W(material: str, espesor_mm: float, kerf_mm: float,
    kerf_top_mm: float,
    kerf_bot_mm: float, velocidad_mm_s: float,
...
def perimetro_entalla_mm(kerf_mm: float, kerf_top_mm: float, kerf_bot_mm: float, espesor_mm: float) -> float:
    """
...
def perimetro_entalla_trapezoidal_mm(kerf_top_mm: float, kerf_bot_mm: float, espesor_mm: float) -> float:
    """
    Perímetro "trapezoidal" de la entalla considerando conicidad:
      P_eff ≈ 2*espesor + kerf_top + kerf_bot
    - kerf_top_mm: ancho de entalla a z=0 (superficie)
    - kerf_bot_mm: ancho de entalla a z=espesor (fondo)
    - espesor_mm:  espesor del material
    """
    return 2.0*espesor_mm + kerf_top_mm + kerf_bot_mm

def phi_jet_ajustado(phi_base, espesor_mm, d_mm, h_mm,
                     a=0.01, b=0.05,                                    # <-- coef. ajuste (heurísticos) -------------------
                     ref_espesor_mm=10.0, ref_d_mm=1.2, ref_h_mm=1.5):
    r_t   = espesor_mm/max(d_mm, 0.4)
    r_h   = h_mm/max(d_mm, 0.4)
    r_t0  = ref_espesor_mm/max(ref_d_mm, 0.4)
    r_h0  = ref_h_mm/max(ref_d_mm, 0.4)
    factor = 1.0 + a*(r_t - r_t0) + b*(r_h - r_h0)
    return max(0.2, min(0.9, phi_base * factor))
...
def balance_energetico_avanzado(setup: CorteSetup, P_bar: float, h_ref_W_m2K: float) -> Dict[str, float]:
    ...
    d = demanda_termica_W(setup.material, setup.espesor_mm, setup.kerf_mm, setup.kerf_top_mm, setup.kerf_bot_mm, setup.velocidad_mm_s, setup.frac_vap)
    ...
    P_perim_mm = perimetro_entalla_trapezoidal_mm(setup.kerf_top_mm, setup.kerf_bot_mm, setup.espesor_mm)
...
def main():
    ...
    # kerf arriba/abajo para perímetro trapezoidal
    kerf_top_mm = kerf_mm
    kerf_bot_um = kerf_um_por_desenfoque(cabezal, delta_f_mm - espesor_mm)
    kerf_bot_mm = kerf_bot_um / 1000.0
    ...
    setup = CorteSetup(
        ...
        kerf_mm=kerf_mm,
        kerf_top_mm=kerf_top_mm,
        kerf_bot_mm=kerf_bot_mm,
    ...
    if not gas.nombre == "O2":
        d_simple = demanda_termica_W(material, espesor_mm, kerf_mm, kerf_top_mm, kerf_bot_mm, setup.velocidad_mm_s, setup.frac_vap)
